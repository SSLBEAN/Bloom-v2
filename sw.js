...
const SHELL_FILES = ['./', './BLOOM_V9_online.html', './manifest.json', './icon-192.png', './icon-512.png'];
...
self.addEventListener('message', (event) => {
  ...
  event.waitUntil(
    self.registration.showNotification(title || 'Bloom', {
      ...
      badge: 'icon-192.png',
      tag: tag || undefined,
      data: { url: url || './BLOOM_V9_online.html' },
    })
  );
});
...
self.addEventListener('notificationclick', (event) => {
  ...
  const targetUrl = (event.notification.data && event.notification.data.url) || './BLOOM_V9_online.html';
  event.waitUntil(
...
self.addEventListener('push', (event) => {
  ...
  event.waitUntil(
    self.registration.showNotification(payload.title || 'Bloom', {
      ...
      icon: 'icon-192.png',
      badge: 'icon-192.png',
      data: { url: payload.url || './BLOOM_V9_online.html' },
    })
  );
});
